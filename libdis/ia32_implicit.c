...
static op_implicit_list_t list_call[] = 
	...
	{{ OP_R | OP_W, REG_EIP_INDEX }, 
	 { OP_R | OP_W, REG_ESP_INDEX }, {0}};	/* call */
...
op_implicit_list_t *op_implicit_list[] = {
	...
	list_rep, list_rsm, list_sahf, list_sgdt,		/* 42 - 44 */
	list_sidt, list_sldt, list_smsw, list_stmxcsr,		/* 44 - 48 */
...
static void handle_impl_reg( x86_op_t *op, unsigned long val ) {
...
unsigned int ia32_insn_implicit_ops( x86_insn_t *insn, unsigned int impl_idx ) {
	...
	for ( list = op_implicit_list[impl_idx]; list->type; list++, num++ ) {
		...
		op->access = (enum x86_op_access) OP_PERM(list->type);
		op->flags = (enum x86_op_flags) (OP_FLAGS(list->type) >> 12);
